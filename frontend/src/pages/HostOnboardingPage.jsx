...
const empty = { displayName: '', headline: '', bio: '', experience: '', languages: [], categories: [], files: [], country: '', payoutMethods: [], deferred: false, commissionAccepted: false };
...
export default function HostOnboardingPage() {
  ...
  const [form, setForm] = useState(() => {
    const stored = JSON.parse(localStorage.getItem('edutalk-host-onboarding') || 'null');
    return stored
      ? { ...empty, ...stored, payoutMethods: stored.payoutMethods || (stored.payoutMethod ? [stored.payoutMethod] : []) }
      : { ...empty, displayName: `${user?.firstName || ''} ${user?.lastName || ''}`.trim() };
  });
  ...
  const valid = useMemo(() => {
    ...
    return form.country && (form.payoutMethods.length || form.deferred) && form.commissionAccepted;
  }, [form, step]);
  const validate = () => {
    ...
    if (step === 3 && !form.payoutMethods.length && !form.deferred) return 'Choose at least one payout method or set up payout details later.';
  ...
  const saveStep = async (nextStep) => {
    ...
    setSaving(true);
    try {
      await api.patch('/auth/host-onboarding', { hostDisplayName: form.displayName.trim(), hostHeadline: form.headline.trim(), hostBio: form.bio.trim(), hostExperience: form.experience, hostLanguages: form.languages, hostCategories: form.categories, hostCredentials: form.files.map(({ name, size, type }) => ({ name, size, type })), payoutCountry: form.country, payoutMethods: form.payoutMethods, payoutDeferred: form.deferred, hostCommissionAccepted: form.commissionAccepted });
  ...
  return <main className="host-onboarding"><header className="host-onboarding-header"><div><p>Host onboarding</p><h1>{title}</h1></div><span>Step {step} of 3</span></header><div className="host-onboarding-progress"><i style={{ width: `${step * 33.333}%` }} /></div><div className="host-onboarding-layout"><section className="host-onboarding-main">
    ...
    {step === 3 && <><label className="host-field">Country of residence<CountrySelect value={form.country} onChange={(value) => { setCountryDetected(false); setField('country', value); }} autoDetect onAutoDetect={setCountryDetected} />{countryDetected && <small>Detected from your location — change if wrong.</small>}</label><div className="payout-field"><strong>Payout methods</strong><div className="payout-cards">{[['paystack', 'Paystack', 'Naira and cedi collections — Nigeria, Ghana, South Africa'], ['stripe', 'Stripe', 'Global card payments, 135+ currencies']].map(([method, label, caption]) => <button type="button" className={form.payoutMethods.includes(method) ? 'is-selected' : ''} key={method} onClick={() => setForm((current) => ({ ...current, payoutMethods: current.payoutMethods.includes(method) ? current.payoutMethods.filter((item) => item !== method) : [...current.payoutMethods, method], deferred: false }))}><b>{label}</b><small>{caption}</small>{form.payoutMethods.includes(method) && <Check size={16} />}</button>)}</div>{form.payoutMethods.length === 2 && <span className="global-reach-pill">Best for global reach</span>}<button type="button" className="later-link" onClick={() => setForm((current) => ({ ...current, payoutMethods: [], deferred: true }))}>Set up payout details later</button></div><div className="plan-review"><h2>Your plan</h2><p>Free plan — up to 3 classes, 25 students per class, 2 sessions per day</p><p>EduTalk keeps 25% of each paid enrollment</p><p>Payouts are released per transaction after a 1–3 day hold</p><a href="/host-plans">See all host plans</a></div><label className="commission-check"><input type="checkbox" checked={form.commissionAccepted} onChange={(event) => setField('commissionAccepted', event.target.checked)} /> <span>I understand EduTalk keeps a 25% commission on paid enrollments and that payouts are released per transaction after a 1–3 day hold.</span></label></>}
  ...
  </section><aside className="host-summary"><h2>Your profile</h2>{[['Display name', form.displayName || '—'], ['Categories', form.categories.length ? form.categories.join(', ') : '—'], ['Languages', form.languages.length ? form.languages.join(', ') : '—'], ['Payout', form.payoutMethods.length ? form.payoutMethods.join(', ') : '—']].map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</aside></div></main>;
}
