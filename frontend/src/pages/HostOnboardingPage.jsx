...
export default function HostOnboardingPage() {
  ...
  return <main className="host-onboarding"><header className="host-onboarding-header"><div><p>Host onboarding</p><h1>{title}</h1></div><span>Step {step} of 3</span></header><div className="host-onboarding-progress"><i style={{ width: `${step * 33.333}%` }} /></div><div className="host-onboarding-layout"><section className="host-onboarding-main">
    ...
    {step === 3 && <><label className="host-field">Country of residence<CountrySelect value={form.country} onChange={(value) => { setCountryDetected(false); setField('country', value); }} autoDetect onAutoDetect={setCountryDetected} />{countryDetected && <small>Detected from your location — change if wrong.</small>}</label><div className="payout-field"><strong>Payout method</strong><div className="payout-cards">{[['Paystack', 'Bank transfer to Nigerian and Ghanaian accounts'], ['Stripe', 'Card payouts to a bank account']].map(([method, caption]) => <button type="button" className={form.payoutMethod === method ? 'is-selected' : ''} key={method} onClick={() => setForm((current) => ({ ...current, payoutMethod: method, deferred: false }))}><b>{method}</b><small>{caption}</small>{form.payoutMethod === method && <Check size={16} />}</button>)}</div><button type="button" className="later-link" onClick={() => setForm((current) => ({ ...current, payoutMethod: '', deferred: true }))}>Set up payout details later</button></div><div className="plan-review"><h2>Your plan</h2><p>Free plan — up to 3 classes, 25 students per class, 2 sessions per day</p><p>EduTalk keeps 25% of each paid enrollment</p><p>Payouts are released per transaction after a 1–3 day hold</p><a href="/host-plans">See all host plans</a></div><label className="commission-check"><input type="checkbox" checked={form.commissionAccepted} onChange={(event) => setField('commissionAccepted', event.target.checked)} /> <span>I understand EduTalk keeps a 25% commission on paid enrollments and that payouts are released per transaction after a 1–3 day hold.</span></label></>}
