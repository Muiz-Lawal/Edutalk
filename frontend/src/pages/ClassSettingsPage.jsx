...
import { Archive, ArrowLeft, Check, Copy, ExternalLink, Link as LinkIcon, Lock, MonitorPlay, Save, Trash2 } from 'lucide-react';
...
export default function ClassSettingsPage() {
  ...
  const [deleteRecordingTarget, setDeleteRecordingTarget] = useState(null);
  ...
  const deleteRecording = async () => {
    ...
    try {
      await api.delete(`/recordings/${deleteRecordingTarget.id}`);
      setHostRecordings((current) => current.map((recording) => recording.id === deleteRecordingTarget.id ? { ...recording, status: 'deleted', isVisible: false } : recording));
      setDeleteRecordingTarget(null);
    } catch {
      setError('The recording could not be deleted. Please retry.');
    }
  };
  ...
  <header className="class-settings-header"><div><button type="button" className="class-settings-back" onClick={() => navigate('/host-dashboard')}><ArrowLeft size={15} /> My Teaching</button><p className="eyebrow">Host workspace</p><h1>{form.title}</h1><div className="class-settings-badges"><Badge variant={form.status === 'active' ? 'success' : 'primary'}>{form.status}</Badge><Badge variant="neutral">{form.isPublic ? 'Public' : 'Unlisted'}</Badge><a href={`/class/${form.appearance.slug || classId}`} target="_blank" rel="noreferrer">View public page <ExternalLink size={14} /></a></div></div><div className="class-settings-actions"><Button variant="secondary" onClick={duplicate}><Copy size={16} /> Duplicate</Button><Button variant="destructive" onClick={archive}><Archive size={16} /> Archive</Button></div></header>
    ...
    <Modal open={Boolean(deleteRecordingTarget)} title="Delete recording" onClose={() => setDeleteRecordingTarget(null)}><p>This removes the recording from student and host access. This cannot be undone.</p><div className="session-modal-actions"><Button variant="secondary" onClick={() => setDeleteRecordingTarget(null)}>Cancel</Button><Button variant="destructive" onClick={deleteRecording}><Trash2 size={14} /> Delete recording</Button></div></Modal>
  </main>;
}
