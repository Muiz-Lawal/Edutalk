...
function ParticipantTile({ participant, local, onPin }) {
  const initials = participant.name.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase();
  return <article className={`session-tile ${participant.speaking ? 'session-tile--speaking' : ''}`}>
...
export default function SessionRoom() {
  ...
  useEffect(() => {
    const onKey = (event) => {
      ...
      if (key === 'r') setHandRaised((value) => !value);
    };
