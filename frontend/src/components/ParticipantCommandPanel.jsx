...
function ParticipantRow({ item, onAction, canManage }) {
  return <article className="command-participant-row">
    ...
    <div className="command-participant-info"><strong>{nameOf(item)}</strong><span>{item.role !== 'student' ? item.role.toUpperCase() : 'STUDENT'}{item.connectionStatus === 'disconnected' && ' · OFFLINE'}</span></div>
...
export default function ParticipantCommandPanel({ roomId, sessionId, isHost, tier, onClose }) {
  ...
  const canManage = Boolean(isHost || data?.isCoHost);
  return <aside className="session-panel command-panel">
    ...
    {!data ? <div className="session-panel__body"><div className="session-skeleton session-skeleton--short" /><div className="session-skeleton session-skeleton--short" /></div> : <div className="command-panel-body">
      ...
      {waiting.length > 0 && <section className="command-waiting"><div className="command-section-title"><h3>Waiting room ({waiting.length})</h3>{canManage && <button type="button" onClick={admitAll}><UserPlus size={14} /> Admit all</button>}</div>{waiting.map((item) => <div className="command-waiting-row" key={item._id}><span>{nameOf(item)}</span>{canManage && <><button type="button" onClick={() => action('admit', item)}>Admit</button><button type="button" onClick={() => setRemoveTarget(item)}>Remove</button></>}</div>)}</section>}
      <section className="command-room-list"><h3>In the room ({participants.length})</h3>{participants.map((item) => <ParticipantRow key={item._id} item={item} canManage={canManage} onAction={(type) => setMenu(type === 'menu' ? item : null)} />)}</section>
