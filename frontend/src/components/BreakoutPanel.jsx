...
export default function BreakoutPanel({ roomId, tier, onClose }) {
  ...
  const finishedCloseAt = useRef(null);

  const load = useCallback(async () => {
    try {
      const response = await api.get(`/video/rooms/${encodeURIComponent(roomId)}/breakouts`);
      const commandCenter = await api.get(`/video/rooms/${encodeURIComponent(roomId)}/command-center`);
      setData(response.data);
      setRoster(commandCenter.data?.participants || []);
      setError('');
      if (response.data?.breakoutRooms?.length) {
        const nextRoomId = response.data.breakoutRooms[0]._id || response.data.breakoutRooms[0].roomId;
        setSelectedRoomId((current) => current || nextRoomId);
      }
    } catch (requestError) {
  ...
  useEffect(() => {
    const closeAt = data?.breakoutState?.closeAt;
    if (data?.breakoutState?.status !== 'closing' || !closeAt || new Date(closeAt).getTime() > Date.now()) return;
    if (finishedCloseAt.current === closeAt) return;
    finishedCloseAt.current = closeAt;
    update('finish_close');
  }, [data?.breakoutState?.closeAt, data?.breakoutState?.status, seconds]);

  const studentOptions = (roster || []).filter((participant) => participant.role === 'student' || (!participant.role || participant.role === 'student'));
  const assignStudent = async () => {
    if (!selectedStudentId || !selectedRoomId) return;
    try {
      await update('assign', { assignments: [{ userId: selectedStudentId, breakoutId: selectedRoomId }] });
      setSelectedStudentId('');
    } catch {
      setError('The student could not be assigned to a breakout room.');
    }
  };
  ...
  return <aside className="session-panel breakout-panel">
    ...
    {!data ? <div className="session-panel__body"><div className="session-skeleton session-skeleton--short" /><div className="session-skeleton session-skeleton--short" /></div> : <div className="command-panel-body">
      ...
      {data.breakoutRooms?.length > 0 && <>
        <div className="breakout-room-list">{data.breakoutRooms.map((room) => <article className="breakout-room-card" key={room._id || room.roomId}><strong>{room.name}</strong><span>{room.participantIds?.length || 0} students · {room.durationMinutes} min</span></article>)}</div>
        <div className="breakout-assignment">
          <label>Student<select value={selectedStudentId} onChange={(event) => setSelectedStudentId(event.target.value)}>
            <option value="">Select a student</option>
            {studentOptions.map((participant) => <option key={participant.userId || participant._id} value={participant.userId || participant._id}>{participant.user ? `${participant.user.firstName || ''} ${participant.user.lastName || ''}`.trim() || participant.user.name || 'Student' : 'Student'}</option>)}
          </select></label>
          <label>Room<select value={selectedRoomId} onChange={(event) => setSelectedRoomId(event.target.value)}>
            {data.breakoutRooms.map((room) => <option key={room._id || room.roomId} value={room._id || room.roomId}>{room.name}</option>)}
          </select></label>
          <button type="button" onClick={assignStudent} disabled={!selectedStudentId || !selectedRoomId}>Assign</button>
        </div>
        <div className="breakout-actions">{data.breakoutState?.status === 'idle' && <><button type="button" onClick={() => update('auto_assign')}>Auto-assign</button><button type="button" onClick={() => update('open')}><DoorOpen size={15} /> Open rooms</button></>}{data.breakoutState?.status === 'open' && <button type="button" onClick={() => update('close')}>Close rooms</button>}{data.breakoutState?.status === 'closing' && <button type="button" onClick={() => update('finish_close')}>Finish return</button>}</div>
        <div className="breakout-broadcast"><textarea maxLength={300} value={announcement} onChange={(event) => setAnnouncement(event.target.value)} placeholder="Announcement to all rooms" /><button type="button" onClick={() => { update('broadcast', { announcement }); setAnnouncement(''); }}><Megaphone size={15} /> Broadcast</button></div>
      </>}
    </div>}
  </aside>;
}
