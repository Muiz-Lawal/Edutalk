...
const pollSchema = new mongoose.Schema({
  ...
  options: [{ _id: mongoose.Schema.Types.ObjectId, label: { type: String, required: true, maxlength: 120 }, votes: { type: Number, default: 0 } }],
  anonymous: { type: Boolean, default: false },
  status: { type: String, enum: ['draft', 'live', 'ended'], default: 'draft' },
  resultsShared: { type: Boolean, default: false },
  voters: [{ type: mongoose.Schema.Types.ObjectId, ref: 'User' }],
  voteDetails: [{ userId: mongoose.Schema.Types.ObjectId, optionId: mongoose.Schema.Types.ObjectId }],
}, { timestamps: true });
...
handSchema.index({ sessionId: 1, loweredAt: 1 });
...
export const SessionHand = mongoose.model('SessionHand', handSchema);
