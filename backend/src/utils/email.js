...
// Robust sendEmail implementation: mock when no key, otherwise use SendGrid via axios
export const sendEmail = async (optionsOrTo, maybeSubject, maybeMessage, maybeTemplate, maybeData = {}) => {
  ...
  try {
    ...
    console.error('[email] Error sending email:', err.response?.data || err.message);
...
export const sendBulkEmail = async (emails, subject, template, data) => {
  ...
  for (const email of emails) {
    const result = await sendEmail({ to: email, subject, template, data });
...
export const emailTemplates = {
  paymentConfirmation: (data) => ({ subject: 'Payment Confirmed - Access Code Inside', data }),
  sessionReminder: (data) => ({ subject: `Class Starting Soon: ${data.className}`, data }),
  subscriptionExpiry: (data) => ({ subject: `Your Subscription to ${data.className} Expires Soon`, data }),
  refundConfirmation: (data) => ({ subject: 'Refund Processed', data }),
};
