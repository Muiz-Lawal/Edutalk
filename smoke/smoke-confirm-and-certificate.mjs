...
async function run() {
  console.log('Starting confirm+certificate smoke tests against', API_BASE);
  ...
  try {
    ...
    classId = body._id || body.id || body.data?._id || null; ok('Create class', !!classId, classId ? `classId=${classId}` : JSON.stringify(body));
  ...
  try {
    ...
    const body = await res.json(); if (!res.ok) throw new Error(JSON.stringify(body)); clientSecret = body.clientSecret || body.client_secret || null; ok('Create payment intent', !!clientSecret, clientSecret ? `amount=${body.amount}` : JSON.stringify(body));
  } catch (err) { ok('Create payment intent', false, err.message); process.exit(6); }

  // Confirm payment (use mock confirm when clientSecret starts with pi_mock_)
  let subscriptionId = null;
  try {
    const paymentIntentId = clientSecret; // in mock path, clientSecret is the paymentIntentId we expect
    ...
    const body = await res.json(); if (!res.ok) throw new Error(JSON.stringify(body));
    subscriptionId = body.subscription?.id || body.subscriptionId || null;
  ...
  // Generate certificate for the subscription (enrollment)
  try {
    ...
    const body = await res.json(); if (!res.ok) throw new Error(JSON.stringify(body));
    const certId = body.data?._id || body.data?.id || body.data?.certificateId || null;
  ...
  console.log('\nConfirm + certificate flow completed successfully.'); process.exit(0);
}
