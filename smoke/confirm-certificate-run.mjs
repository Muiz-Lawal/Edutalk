...
async function run() {
  ...
  // Create payment intent - note: backend may mock this when Stripe not configured
  ...
  // Confirm payment (backend may create subscription directly when mocking)
  ...
  try {
    ...
    const body = await res.json(); if (!res.ok) throw new Error(JSON.stringify(body));
    subscriptionId = body.subscription?.id || body.subscriptionId || body.subscription?._id || body.data?._id || null;
